...
public class GetCapabilitiesActivity extends Activity 
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		try 
		{
			...
			//check PTAPI GUID
			try{
				byte[] guid = reader.GetParameter(Reader.ParamId.PARMID_PTAPI_GET_GUID);
				if(16 == guid.length){
		...
		String[] values = new String[21 + caps.resolutions.length];
		...
		values[20] = "Eikon persistent serial number: " + strPtapiGuid;

		for(int nCount = 0; nCount < caps.resolutions.length; nCount++)
		{
			values[21 + nCount] = "Resolution (" + (nCount + 1) + " of " + caps.resolutions.length + "): " + String.valueOf(caps.resolutions[nCount]);
		}
